 ...
 * 
 * Enhanced Features:
 * - Added more chat commands: !odds, !hoststats, !top, !balance, !limits, !games
 ...
 * - !balance shows the host's current coin balance in inventory
 * - !limits shows min/max bet limits
 * - !games shows available games and their status
 * 
 * @author ikingsnipe
 * @version 2.4.0
 */
...
public class EliteTitanCasino extends AbstractScript implements ChatListener {
    ...
    private class ChatParser {
        ...
        private final Pattern BALANCE_PATTERN = Pattern.compile("!balance", Pattern.CASE_INSENSITIVE);
        private final Pattern LIMITS_PATTERN = Pattern.compile("!limits", Pattern.CASE_INSENSITIVE);
        private final Pattern GAMES_PATTERN = Pattern.compile("!games", Pattern.CASE_INSENSITIVE);
        ...
        void processMessage(String sender, String message) {
            ...
            try {
                ...
                Matcher limitsMatcher = LIMITS_PATTERN.matcher(message);
                if (limitsMatcher.matches()) {
                    handleLimits(sender);
                    return;
                }
                
                Matcher gamesMatcher = GAMES_PATTERN.matcher(message);
                if (gamesMatcher.matches()) {
                    handleGames(sender);
                    return;
                }
        ...
        private void handleBalance(String player) {
            long balance = Inventory.count(COINS_ID);
            String message = String.format("🏦 Host Balance: %s GP available for payouts.", formatCoins((int) balance));
            sendMessage(player, message);
        }
        
        private void handleLimits(String player) {
            String message = String.format("📏 Betting Limits: Min %s GP | Max %s GP", 
                formatCoins(config.minBet), formatCoins(config.maxBet));
            sendMessage(player, message);
        }
        
        private void handleGames(String player) {
            StringBuilder sb = new StringBuilder("🎲 Available Games:\n");
            for (String game : Arrays.asList("craps", "dice", "flower")) {
                GameEngine engine = getGameEngine(game);
                if (engine != null) {
                    sb.append(game.toUpperCase()).append(": ").append(engine.isEnabled() ? "Enabled" : "Disabled").append("\n");
                }
            }
            sendMessage(player, sb.toString());
        }
        ...
        private void handleHelp(String player) {
            String help = "🎰 Elite Titan Casino Commands:\n" +
                          ...
                          "!balance - View host's current balance\n" +
                          "!limits - View betting limits\n" +
    ...
    public void onStart() {
        log("================================================");
        log("     ELITE TITAN CASINO PRO ENHANCED v2.4");
        log("================================================");
    ...
    private void createGUI() {
        try {
            controlFrame = new JFrame("🎰 Elite Titan Casino Pro Enhanced v2.4");
    ...
    private JPanel createStatusBar() {
        ...
        JLabel versionLabel = new JLabel(" Elite Titan Casino Pro Enhanced v2.4 | DreamBot 4.0+ ");
